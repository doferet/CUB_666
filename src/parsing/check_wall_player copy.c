...
/*   Updated: 2025/04/22 01:57:55 by rbalazs          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((cub->map.map[y][x] == '0' || is_player(cub, x, y))
		&& cub->map.map[y][x - 1] == ' ')
		ft_error(cub, "Walls are not valid or player is outside");
	if ((cub->map.map[y][x] == '0' || is_player(cub, x, y))
		&& cub->map.map[y][x + 1] == ' ')
		ft_error(cub, "Walls are not valid or player is outside");
	if ((cub->map.map[y][x] == '0' || is_player(cub, x, y))
...
static bool	check_begin_end(t_cub *cub, int x, int y, bool start)
{
	if (cub->map.map[y][x] == '1')
		start = true;
	if ((cub->map.map[y][x] == '0' || is_player(cub, x, y)) && start == false)
		ft_error(cub, "Walls are not valid or player is outside");
	if ((cub->map.map[y][(int)ft_strlen(cub->map.map[y]) - 1] == '0'
		|| is_player(cub, (int)ft_strlen(cub->map.map[y]) - 1, y)))
		ft_error(cub, "Walls are not valid or player is outside");
	return (start);
}
...
void	check_first_and_last_line(t_cub *cub)
{
	...
	{
		if (cub->map.map[0][x] == '0' || is_player(cub, x, 0))
			ft_error(cub, "Walls are not valid or player is outside");
	}
	x = -1;
	while (++x < (int)ft_strlen(cub->map.map[cub->map.rows - 1]))
	{
		if (cub->map.map[cub->map.rows - 1][x] == '0'
			|| is_player(cub, x, cub->map.rows - 1))
			ft_error(cub, "Walls are not valid or player is outside");
	}
}
